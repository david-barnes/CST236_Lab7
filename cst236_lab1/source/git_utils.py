""" Source file for git_utils functions """
...
# pylint hates global constants and I don't see a problem here
git_logger = logging.getLogger(__name__)


def check_valid_path(func):
    """ checks to see if the given file has a valid path """
    @wraps(func)
...
def is_file_in_repo(path):
    """ checks to see if the given file is in the repo """
...
@check_valid_path
def get_file_info(path, dummy=False):
    """
...
def git_execute(params=None, path=os.getcwd()):
    ...
    if stderr:
        git_logger.error('Error occurred when executing git command(%s): %s', params,
                         stderr.strip())
    print 'git_execute {}'.format(params)
